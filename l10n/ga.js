OC.L10N.register(
    "rolls",
    {
    "Rolls" : "Rollaí",
    "Share quick videos with your colleagues" : "Roinn físeáin thapa le do chomhghleacaithe",
    "This plugin allows you to create screen recordings and share them with anyone withing your organization." : "Ligeann an breiseán seo duit taifeadtaí scáileáin a chruthú agus iad a roinnt le haon duine laistigh de d'eagraíocht.",
    "New roll" : "Rolla nua",
    "No comments yet" : "Níl trácht ar bith fós",
    "Start writing comments and they will appear here." : "Tosaigh ag scríobh tuairimí agus beidh siad le feiceáil anseo.",
    "Type a comment" : "Clóscríobh nóta tráchta",
    "Editing" : "Eagarthóireacht",
    "Edit comment" : "Cuir trácht in eagar",
    "Delete comment" : "Scrios nóta tráchta",
    "Add\") : t(\"rolls\", \"Update" : "Add\") : t( \"rollaí\", \"Nuashonraigh",
    "Discard" : "Caith amach",
    "No rolls yet" : "Níl rollaí fós",
    "Start by creating a new roll using the button above." : "Tosaigh trí rolla nua a chruthú ag baint úsáide as an gcnaipe thuas.",
    "Created by" : "Cruthaithe ag",
    "Do you really want to delete this Roll?" : "An bhfuil tú cinnte gur mhaith leat an Rolla seo a scriosadh?",
    "Your browser does not support Picture in Picture, some features are disabled." : "Ní thacaíonn do bhrabhsálaí le Pictiúr i bPictiúr, tá roinnt gnéithe díchumasaithe.",
    "Rolls is not available on mobile devices" : "Níl Rollaí ar fáil ar ghléasanna soghluaiste",
    "Disabled" : "Faoi mhíchumas",
    "No microphones found" : "Níor aimsíodh micreafón",
    "An unsaved video was found and can be recovered" : "Fuarthas físeán gan sábháil agus is féidir é a aisghabháil",
    "Uploading..." : "Á uaslódáil...",
    "Recording…" : "Ag taifeadadh…",
    "Your screen is being recorded" : "Tá do scáileán á thaifeadadh",
    "Select a screen to get started" : "Roghnaigh scáileán chun tús a chur leis",
    "If you decide to share a window only, the webcam Picture in Picture will not be available" : "Má shocraíonn tú gan ach fuinneog a roinnt, ní bheidh an ceamara gréasáin Pictiúr i bPictiúr ar fáil",
    "Name or email" : "Ainm nó ríomhphost",
    "Search for share recipients" : "Cuardaigh faighteoirí scaireanna",
    "Add" : "Cuir",
    "Delete" : "Scrios",
    "Your browser does not support videos." : "Ní thacaíonn do bhrabhsálaí le físeáin.",
    "Add a title" : "Cuir teideal leis",
    "Add a comment at this timestamp" : "Cuir nóta tráchta leis ag an stampa ama seo",
    "Try to upload again" : "Bain triail as a uaslódáil arís",
    "Start recording" : "Tosaigh taifeadadh",
    "An error occurred while uploading your Roll" : "Tharla earráid agus do Rolla á uaslódáil",
    "Error" : "Earráid",
    "description" : "tuairisc",
    "comments" : "tuairimí",
    "This roll was not found" : "Ní bhfuarthas an rolla seo",
    "Download" : "Íoslódáil",
    "Copy URL" : "Cóipeáil URL",
    "Copied!" : "Cóipeáladh!",
    "Share" : "Comhroinn",
    "No description provided" : "Níor tugadh tuairisc"
},
"nplurals=5; plural=(n==1 ? 0 : n==2 ? 1 : n<7 ? 2 : n<11 ? 3 : 4);");
