<template>
	<div class="tw-flex tw-flex-row tw-items-center tw-gap-2">
		<NcPopover>
			<template #trigger>
				<NcButton :disabled="!ready"> Change webcam </NcButton>
			</template>
			<template>
				<ul v-if="webcamDevices.length" class="tw-flex tw-flex-col tw-items-start tw-p-1">
					<li v-for="dev in webcamDevices" :key="dev.deviceId">
						<NcButton type="tertiary" @click="$emit('webcamChange', dev)">
							<template #icon>
								<CheckBold :size="20" v-if="dev.deviceId === activeWebcamId" class="tw-text-green-500"/>
		...
		<NcPopover>
			<template #trigger>
				<NcButton :disabled="!ready"> Change micrphone </NcButton>
			</template>
			<template>
